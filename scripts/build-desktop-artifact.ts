...
export const DESKTOP_FILE_EXCLUSIONS = [
  // Ronin always passes the user's installed Claude executable to the SDK,
...
// remain archived instead of inflating the Windows install's loose-file count.
...
export function createStageWorkspaceConfig(input: {
  ...
  // Linux AppImages load glibc Node optional deps at runtime (e.g.
  // @yuuang/ffi-rs-linux-x64-gnu). Keep libc explicit so pnpm includes those
  // packages in the staged production install.
...
export const stageResourceMonitor = Effect.fn("stageResourceMonitor")(function* (input: {
  ...
  // Release CI restores the compiled binary from a cache keyed on the crate's
  // sources, so the toolchain never has to be installed on a hit. The staging
  // and existence checks below still run: a corrupt or missing restore fails
  // here rather than shipping an artifact without a resource monitor.
  const reuseResourceMonitor = yield* Config.boolean("T3CODE_DESKTOP_REUSE_RESOURCE_MONITOR").pipe(
    Config.withDefault(false),
  );
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "Ronin (Nightly)"
    : (desktopPackageJson.productName ?? "Ronin");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    // Plain string on purpose: ${version}/${arch}/${ext} are electron-builder
    ...
    buildConfig.mac = {
      ...
      extendInfo: {
        NSScreenCaptureUsageDescription:
          "Ronin captures the active window when you use the window capture shortcut.",
      },
      protocols: [
        {
          name: "Ronin",
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  Command.withDescription("Build a desktop artifact for Ronin."),
