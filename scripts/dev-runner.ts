...
export const DEFAULT_T3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), DEFAULT_RONIN_HOME_DIRNAME),
);
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .ronin > ambient T3CODE_HOME) is resolved
    ...
    // agent working inside Ronin), these leak through and the child server
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    // (gitignored) `.ronin` — see @t3tools/shared/devHome for why this must
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Ronin data directory; runtime state is stored under userdata (equivalent to T3CODE_HOME). Inside a git worktree this defaults to that worktree's own .ronin so dev state stays off the shared home.",
    ),
    Flag.optional,
