...
describe("third-party license generation", () => {
  it("references only notice files present in the desktop fork", async () => {
    const config = JSON.parse(
      ...
      readonly customNotices: ReadonlyArray<{ readonly noticeFile?: string }>;
      readonly packageOverrides: ReadonlyArray<{ readonly noticeFile?: string }>;
    };
    const noticeFiles = [...config.customNotices, ...config.packageOverrides].flatMap((entry) =>
      entry.noticeFile ? [entry.noticeFile] : [],
    );

    expect(noticeFiles.some((path) => path.startsWith("apps/mobile/"))).toBe(false);
    await Promise.all(
      noticeFiles.map((path) => NodeFSP.access(NodePath.join(REPOSITORY_ROOT, path))),
    );
  });
