...
describe("loadRepoEnv", () => {
  it("does not project optional public configuration for an unconfigured clone", () => {
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).T3CODE_HOSTED_APP_URL).toBe(
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        VITE_HOSTED_APP_URL: "https://legacy.example.test",
      ...
      hostedAppUrl: "https://legacy.example.test",
