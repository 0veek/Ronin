...
it("builds a prerelease Discord announcement for nightly subscribers", () => {
  assert.deepStrictEqual(
    buildDiscordReleaseAnnouncement({
      ...
      releaseName: "Ronin Nightly 1.2.4-nightly.20260501.17 (abcdef123456)",
    ...
    {
      content:
        "<@&111111111111111111> Prerelease published: Ronin Nightly 1.2.4-nightly.20260501.17 (abcdef123456)",
      allowed_mentions: {
        roles: ["111111111111111111"],
      },
      embeds: [
        {
          title: "Ronin Nightly 1.2.4-nightly.20260501.17 (abcdef123456)",
          url: "https://github.com/t3dotgg/t3-code/releases/tag/v1.2.4-nightly.20260501.17",
          description: "A new Ronin prerelease is available for nightly testers.",
          color: 0x5865f2,
          fields: [
            {
              name: "Version",
...
it("builds a latest Discord announcement for stable subscribers", () => {
  assert.deepStrictEqual(buildDiscordReleaseAnnouncement(latestAnnouncement), {
    content: "<@&222222222222222222> Latest published: Ronin v1.2.3",
    allowed_mentions: {
    ...
    embeds: [
      {
        ...
        description: "A new Ronin latest release is available.",
        color: 0x2ecc71,
        fields: [
          {
            name: "Version",
