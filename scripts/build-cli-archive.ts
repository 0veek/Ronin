 ...
 * archive: the `t3` binary, the web client, bundled skills, the resource monitor, and a
...
const buildCliArchive = Effect.fn("buildCliArchive")(function* (input: {
  ...
  const bundledSkills = path.join(serverDir, "dist/skills");
  ...
  yield* requireInput(bundledSkills, "Run `vp run --filter t3 build` first.");
  yield* requireInput(
    resourceMonitorDir,
  ...
  yield* fs.copy(bundledSkills, path.join(contentDir, "skills"));
