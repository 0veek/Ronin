...
function mockProcess(exitCode: number, stdout = "") {
  ...
  return ChildProcessSpawner.makeHandle({
    ...
    stdin: Sink.drain,
    stdout: stdout === "" ? Stream.empty : Stream.make(encodedStdout),
    stderr: Stream.empty,
    all: stdout === "" ? Stream.empty : Stream.make(encodedStdout),
    getInputFd: () => Sink.drain,
    getOutputFd: () => (stdout === "" ? Stream.empty : Stream.make(encodedStdout)),
  });
}
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Ronin");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Ronin (Nightly)");
  });
  ...
  it("unpacks native binaries while keeping their JavaScript and metadata archived", () => {
    for (const file of [
      ...
      assert.isTrue(NodePath.matchesGlob(file, WINDOWS_NATIVE_ASAR_UNPACK_GLOB));
    }
    ...
    for (const file of [
      ...
      assert.isFalse(NodePath.matchesGlob(file, WINDOWS_NATIVE_ASAR_UNPACK_GLOB));
    }
  });
