...
import { SpeechToTextProvider, SpeechToTextSettings } from "./speechToText.ts";
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let Ronin spawn the server when needed.",
        providerSettingsForm: {
...
export const AntigravitySettings = makeProviderSettingsSchema(
  {
    ...
    binaryPath: makeBinaryPathSetting("agy").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the Antigravity CLI (`agy`).",
...
export const DroidSettings = makeProviderSettingsSchema(
  {
    ...
    binaryPath: makeBinaryPathSetting("droid").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the Factory Droid CLI.",
...
export const PiSettings = makeProviderSettingsSchema(
  {
    ...
    binaryPath: makeBinaryPathSetting("pi").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description:
          "Path to the Pi coding-agent CLI. Leave as `pi` when using the bundled library.",
    ...
    agentDir: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        description: "Optional Pi agent/session directory. Leave blank for the default.",
        providerSettingsForm: {
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  // No apiKey member: keys go to the secret store through their own RPC, so a
  // settings patch can never carry one into settings.json.
  speechToText: Schema.optionalKey(
    Schema.Struct({
      enabled: Schema.optionalKey(Schema.Boolean),
      provider: Schema.optionalKey(SpeechToTextProvider),
      models: Schema.optionalKey(
        Schema.Struct({
          deepgram: Schema.optionalKey(TrimmedString),
          groq: Schema.optionalKey(TrimmedString),
          openrouter: Schema.optionalKey(TrimmedString),
        }),
      ),
