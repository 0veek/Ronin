...
const legacyThreadLinkFields = Schema.Struct({
  linkedPullRequest: Schema.optional(ThreadLinkedPullRequest),
});
const decodeLegacyThreadLinkFields = Schema.decodeUnknownEffect(legacyThreadLinkFields);
...
it.effect("defaults settled fields when decoding historical thread data", () =>
  Effect.gen(function* () {
    ...
    const oldClientFields = yield* decodeLegacyThreadLinkFields(newServerWire);
