...
describe("environment RPC", () => {
  ...
  it.effect("observes unary requests until they complete", () =>
    Effect.gen(function* () {
      ...
      const result = yield* request(WS_METHODS.serverGetConfig, void 0 as never).pipe(
        Effect.provideService(EnvironmentSupervisor.EnvironmentSupervisor, supervisor),
  ...
  it.effect("binds finite streaming commands to one active session", () =>
    Effect.gen(function* () {
      const firstEvents = yield* Queue.unbounded<string>();
      const secondEvents = yield* Queue.unbounded<string>();
      const firstClient = {
      ...
      yield* Queue.offer(firstEvents, "checking");
      yield* SubscriptionRef.set(activeSession, Option.some(session(secondClient)));
      yield* Queue.offer(secondEvents, "downloading");
      yield* Queue.offer(firstEvents, "downloading");

      expect(yield* Fiber.join(resultFiber)).toEqual(["checking", "downloading"]);
    }),
  );
