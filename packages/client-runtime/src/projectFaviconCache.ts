...
/**
 * The image type of a favicon URL, read from its extension. Only used when the
 * response omits an image `content-type`; project icons are always one of the
 * workspace image formats.
 */
...
function faviconMimeType(url: string): string | null {
  const dataMimeType = /^data:(image\/[\w.+-]+)[;,]/i.exec(url)?.[1];
  if (dataMimeType) return dataMimeType.toLowerCase();
  let pathname = url.split(/[?#]/, 1)[0] ?? "";
  try {
    pathname = decodeURIComponent(new URL(url, "https://icon.invalid").pathname);
  } catch {
    // Not a URL, or a literal percent in the filename: fall back to the raw path.
  }
  const basename = pathname.split(/[\\/]/).at(-1) ?? "";
  const extensionIndex = basename.lastIndexOf(".");
  if (extensionIndex < 0) return null;
  return IMAGE_MIME_TYPE_BY_EXTENSION.get(basename.slice(extensionIndex).toLowerCase()) ?? null;
}
...
export function createProjectFaviconImageLoader(input: {
  ...
  return async (url: string, signal: AbortSignal): Promise<string> => {
    ...
    const contentType = response.headers
      ...
      ?.trim()
      .toLowerCase();
    const mimeType = contentType?.startsWith("image/") ? contentType : faviconMimeType(url);
