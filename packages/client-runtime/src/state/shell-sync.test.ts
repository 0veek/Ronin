...
describe("environment shell synchronization", () => {
  ...
  // applyItems coalesces a chunk into one SubscriptionRef write. This fork's
  // subscribeDynamic taps per event, so a fromQueue fake yields one-item chunks
  // and cannot pin RpcClient's 16-event buffer sizes. The assertion is the
  // end state: every upsert lands, in order, at snapshotSequence 51.
  it.live("applies a burst of live thread upserts without dropping events", () =>
    Effect.gen(function* () {
      const events = yield* Queue.bounded<OrchestrationShellStreamItem>(Infinity);
      const client = {
        [ORCHESTRATION_WS_METHODS.subscribeShell]: () => Stream.fromQueue(events),
      } as unknown as WsRpcProtocolClient;
      const supervisorState = yield* SubscriptionRef.make(AVAILABLE_CONNECTION_STATE);
      ...
      expect(snapshots.at(-1)!.snapshotSequence).toBe(51);
