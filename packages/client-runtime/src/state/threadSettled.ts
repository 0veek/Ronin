...
/**
 * Whether a thread accepts a settle right now.
 *
 * The server owns settlement and rejects an ineligible settle with
 * `OrchestrationThreadSettleBlockedError`, so this is only a pre-check: it
 * lets the board grey the action out instead of offering a menu entry whose
 * only outcome is an error toast. The blockers mirror the decider's.
 */
