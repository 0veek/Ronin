...
DEFAULT_SERVER_HOME="$HOME/.ronin"
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  if ! wait_ready "@@T3_READY_TIMEOUT_MS@@"; then
    printf 'Remote Ronin server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
if [ "$REMOTE_MANAGED" != "external" ] && [ -n "$REMOTE_PID" ] && kill -0 "$REMOTE_PID" 2>/dev/null; then
  ...
  if kill -0 "$REMOTE_PID" 2>/dev/null; then
    printf 'Remote Ronin server with PID %s did not stop within 2 seconds. Its ownership files were kept.\\n' "$REMOTE_PID" >&2
    exit 1
  fi
fi
rm -f "$PID_FILE" "$PORT_FILE" "$MANAGED_FILE"
