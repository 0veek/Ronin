...
if ! t3_runtime_ready; then
  ...
  # under it.
  T3_LOCK="$HOME/.ronin/runtime/versions/.$T3_ARCHIVE_VERSION.install.lock"
...
if ! t3_runtime_ready; then
  ...
  T3_STAGING="$(mktemp -d "$HOME/.ronin/runtime/versions/.staging-XXXXXX")"
...
@@T3_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.ronin/ssh-launch/$STATE_KEY"
...
STATE_DIR="$HOME/.ronin/ssh-launch/@@T3_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.t3"
RUNNER_FILE="$STATE_DIR/run-t3.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
