...
describe("ssh tunnel scripts", () => {
  ...
  it("uses the remote t3 runner for launch and pairing scripts", () => {
    ...
    assert.include(buildRemoteLaunchScript(), "Remote Ronin server did not become ready");
          ...
          if (args.includes("sh")) {
            ...
            if (mode === "failed stop" && stopCommandCount === 1) {
              return {
                ...
                stderr: Stream.make(
                  new TextEncoder().encode("Remote Ronin server did not stop within 2 seconds.\n"),
                ),
              };
            }
        ...
        if (mode === "failed stop") {
          ...
          if (Result.isFailure(disconnected)) {
            ...
            assert.equal(
              ...
              "Remote Ronin server did not stop within 2 seconds.",
            );
          }
        } else {
