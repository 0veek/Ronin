...
describe("ssh tunnel scripts", () => {
  it("installs and runs the release archive without Node, npm, or npx", () => {
    ...
    assert.include(
      script,
      "T3_RELEASE_BASE_URL='https://github.com/0veek/Ronin/releases/download'",
    );
    assert.include(script, 'T3_RUNTIME_DIR="$HOME/.ronin/runtime/versions/$T3_ARCHIVE_VERSION"');
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const versionsDir = `${home}/.ronin/runtime/versions`;
