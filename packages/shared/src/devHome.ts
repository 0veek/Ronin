 ...
 * `~/.ronin` that a user's installed Ronin runs against.
 *
 * A linked git worktree gets its own (gitignored) `.ronin`: feature work in a
...
import { WORKTREE_HOME_DIRNAME } from "./roninHome.ts";
...
export const resolveWorktreeT3Home = (
  ...
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, WORKTREE_HOME_DIRNAME);
  });
