...
describe("worktree setup visibility", () => {
  const setupThreadId = ThreadId.make("thread-setup");
  const setupNow = "2026-03-29T00:00:00.000Z";
  const stage = (
    ...
    status,
    startedAt: setupNow,
    endedAt: status === "running" || status === "pending" ? null : setupNow,
    percent: null,
    detail: null,
    tail: [],
  });
  const base = {
    threadId: setupThreadId,
  ...
  it("reads the settled snapshot back from the thread's activities", () => {
    ...
    expect(findRecordedWorktreeSetup(activities, setupThreadId)).toEqual(settledDone);
...
describe("buildRunningThreadTurnInterruptInput", () => {
  it("targets only a running thread", () => {
    const activeTurnId = TurnId.make("turn-running");
    const running = makeThread({
      session: { ...readySession, status: "running", activeTurnId },
    });
    expect(buildRunningThreadTurnInterruptInput(running, "running")).toEqual({
      threadId,
      turnId: activeTurnId,
    });
    expect(buildRunningThreadTurnInterruptInput(running, "ready")).toBeNull();
