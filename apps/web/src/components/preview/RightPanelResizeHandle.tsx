...
export function RightPanelResizeHandle({ handlers, className }: Props) {
  return (
    ...
    >
      <span
        aria-hidden
        className="pointer-events-none absolute inset-y-0 left-1/2 w-px -translate-x-1/2 bg-transparent transition-colors duration-(--duration-fast) group-hover:bg-border group-active:bg-primary/60"
      />
    </div>
  );
}
