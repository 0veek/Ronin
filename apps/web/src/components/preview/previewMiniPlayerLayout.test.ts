...
describe("resolveDeviceMiniPlayerCornerRadius", () => {
  it("rounds an Android player like a phone, scaled with its short side", () => {
    ...
    expect(resolveDeviceMiniPlayerCornerRadius("android", { width: 60, height: 130 })).toBe(8);
  });

  it("keeps the frame radius for iOS, whose stream has square corners", () => {
    expect(resolveDeviceMiniPlayerCornerRadius("ios", { width: 240, height: 520 })).toBe(8);
    expect(resolveDeviceMiniPlayerCornerRadius("ios", { width: 720, height: 1_000 })).toBe(8);
  });
});
