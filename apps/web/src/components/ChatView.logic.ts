...
/**
 * The new-turn anchor parks the just-sent message at the top of the viewport
 * and waits for the reply to grow under it. Work-log rows are not messages, so
 * a turn that opens with tool calls never grows anything the anchor can see:
 * the anchored message scrolls off, and the user is left staring at blank
 * space while the agent works. Once the running turn emits anything tool-like,
 * the anchor has lost its argument and live-follow takes back over.
 */
