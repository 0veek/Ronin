...
function TimelineLoadEarlierHeader({
  ...
  return (
    <div className={fade ? "pt-[var(--workspace-titlebar-scroll-fade-height)]" : "pt-3 sm:pt-4"}>
      <div className="mx-auto w-full max-w-[var(--app-chat-max-width,48rem)] pb-2">
        <button
          type="button"
...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  const handleScroll = useCallback(() => {
    ...
    for (const [index, item] of minimapItems.entries()) {
      ...
      if (strip) strip.dataset.inView = inView ? "true" : "false";
    }
  ...
  const renderItem = useCallback(
    ({ item }: { item: MessagesTimelineRow }) => (
      <div
        className="mx-auto w-full min-w-0 max-w-[var(--app-chat-max-width,48rem)] overflow-x-clip"
    ...
    <Tooltip>
      <TooltipTrigger
        render={
          <span
            className={cn(
              "pointer-events-auto absolute left-3 z-10 inline-flex -translate-x-1/2 opacity-0 transition-opacity duration-(--duration-fast) hover:opacity-100 focus-within:opacity-100",
              previous ? "top-2" : "bottom-2",
            )}
          />
        }
      >
...
// Expose transcript authors as headings without changing the visual layout or
// copied text. Markdown headings inside each message nest beneath this level.
...
function UserTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "message" }> }) {
  ...
  const userFiles = useMemo(
    () => (messageWithPreviews.attachments ?? []).filter(isFileAttachment),
    [messageWithPreviews.attachments],
  );
  const userVideos = userFiles.filter(isVideoAttachment);
  const otherUserFiles = userFiles.filter((file) => !isVideoAttachment(file));
  const unknownAttachments = (messageWithPreviews.attachments ?? []).filter(
  ...
  return (
    <>
      <div className="relative min-w-0 px-1 py-0.5">
        <MessageAuthorHeading>Ronin</MessageAuthorHeading>
        {showsSource ? (
...
const TurnPlanTimelineRow = memo(function TurnPlanTimelineRow({
  ...
  return (
    <div className="min-w-0 px-1 py-0.5">
      <button
        type="button"
        className="flex w-full min-w-0 cursor-pointer items-center gap-2 rounded-md px-0.5 py-0.5 text-left text-xs leading-5 transition-colors duration-(--duration-fast) hover:bg-accent/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring/70"
...
function UserMessagePullRequestContextChip(props: {
  ...
  const ctx = use(TimelineRowCtx);
  const openPullRequest = useOpenChangeRequestLink(ctx.threadRef ?? undefined);
  ...
  return (
    <div className="max-w-full overflow-hidden rounded-lg border border-border/70 bg-background/70">
      {props.image?.previewUrl ? (
        ...
        >
          <FallbackImage
            ...
            fallback={<MissingMediaBlock label="Screenshot unavailable" className="min-h-24" />}
          />
        </button>
      ) : (
      ...
      <div className="min-w-0 px-3 py-2.5">
        ...
        {props.record.targetSummary || (props.record.styleChanges?.length ?? 0) > 0 ? (
          <div className="mt-1 flex items-center gap-2 text-secondary-label text-3xs">
            {props.record.targetSummary ? <span>{props.record.targetSummary}</span> : null}
    ...
    },
    {
      ...
      render: (record, context) => {
        ...
        return (
          <ImageChipButton
            name={record.name}
            previewUrl={context.attachment.previewUrl}
            className={CHAT_INLINE_CHIP_CLASS_NAME}
            labelClassName={CHAT_INLINE_CHIP_LABEL_CLASS_NAME}
            size={formatAttachmentSize(record.sizeBytes)}
            data-markdown-copy={context.copyMarkdown}
            onClick={() => context.onExpandImage(context.attachment as ChatImageAttachment)}
          />
        );
      },
    },
      ...
      render: (record, context) => {
        ...
        return (
          <FileChip
            ...
            className={CHAT_INLINE_CHIP_CLASS_NAME}
            labelClassName={CHAT_INLINE_CHIP_LABEL_CLASS_NAME}
            disabled={attachment.downloadable === false}
            accessibleLabel={`${isVideo ? "Video" : "File"} attachment, ${record.name}, ${size}`}
            copyMarkdown={context.copyMarkdown}
            onOpen={() => context.onOpenFile(attachment)}
            tooltip={`${record.name}\n${size}`}
          />
        );
      },
    ...
    },
    {
      ...
      render: (record, context) => {
        ...
        return (
          <UserMessageContextPopover
            copyMarkdown={context.copyMarkdown}
            accessibleLabel={`${kindLabel}, ${label}`}
            chip={
              <UserMessageContextChip
                icon={
                  <MessageCircleIcon
      ...
      <Fragment key={key}>
        ...
        {content ? (
          <ChatMarkdown
            ...
            parseRawHtml={false}
            renderContextReference={props.renderContextReference}
            headingLevelOffset={props.headingLevelOffset}
          />
        ) : null}
...
function workEntryIconName(workEntry: TimelineWorkEntry): WorkEntryIconName {
  ...
  const nativeToolPresentation = resolveWorkEntryToolPresentation(workEntry);
  if (nativeToolPresentation?.icon === "pull-request") {
    return "pull-request";
  }
  if (nativeToolPresentation?.icon === "device") return "device";
...
/** One expandable timeline row per subagent spawn batch. */
const AgentSpawnRow = memo(function AgentSpawnRow(props: { workEntry: TimelineWorkEntry }) {
  ...
  return (
    <div className="flex flex-col">
      ...
      {expanded ? (
        <div className="ms-6 mt-1 flex flex-col gap-0.5 border-s border-border/60 ps-2">
          {agents.map((agent) => (
...
function AgentSpawnMemberRow({ agent }: { agent: RuntimeSubagent }) {
  ...
  const status =
    tokens > 0
      ? `${AGENT_MEMBER_STATUS_LABEL[agent.status]} · ${formatSubagentTokenCount(tokens)} tok`
  ...
  const body = [activity?.trim() || null, model || null].filter(Boolean).join("\n\n");
  ...
  return (
    <button
      ...
      className="flex min-w-0 flex-col rounded-md px-1 py-0.5 text-left enabled:hover:bg-accent/20 disabled:cursor-default"
    >
      <span className="flex w-full min-w-0 items-baseline gap-1.5">
        <span
          className={cn(
            "min-w-0 flex-1 truncate text-sm",
            agent.status === "failed" && "text-error-foreground",
          )}
        >
          {agent.title}
        </span>
      ...
      {!open && activity ? (
        <span className="w-full truncate text-xs text-muted-foreground">{activity}</span>
      ) : null}
      {open ? (
        <pre className="mt-1 max-h-64 w-full cursor-text overflow-auto whitespace-pre-wrap break-words rounded-md bg-muted/40 px-3 py-2 font-mono text-2xs leading-relaxed text-secondary-label select-text">
          {body}
        </pre>
      ) : null}
    </button>
...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  const preview =
    ...
    normalizeCompactToolLabel(rawPreview).toLowerCase() ===
      ...
      ? null
      : rawPreview;
  const collapsedText = preview ? `${heading} - ${preview}` : heading;
  ...
  const accessibleDisplayText = [displayText, answerPreview].filter(Boolean).join(": ");
            ...
            <span className="flex size-4 shrink-0 items-center justify-center">
              {showFailedIndicator ? (
                <Tooltip>
                  ...
                  >
                    <CircleAlertIcon
                      className="block size-3 shrink-0 text-destructive"
                      aria-hidden
                    />
                  </TooltipTrigger>
