...
function ComposerServerUpdateIcon({ status }: { readonly status: ServerUpdateState["status"] }) {
  if (status === "running") return <Spinner aria-hidden />;
  if (status === "failed") return <CircleAlertIcon aria-hidden className="text-error" />;
