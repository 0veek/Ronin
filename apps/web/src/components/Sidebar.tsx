...
function SidebarSectionHeader(props: {
  ...
  const className = cn(
    ...
    snoozed
      ? "text-status-snoozed-foreground"
      : needsYou
  ...
  const ruleClassName = cn(
    "h-px min-w-2 flex-1",
    snoozed ? "bg-status-snoozed/20" : needsYou ? "bg-status-attention/20" : "bg-sidebar-border/60",
  ...
  return (
    ...
    >
      {props.toggle ? (
        ...
        <h2 data-testid="sidebar-needs-you-header" className={className}>
          {content}
        </h2>
      )}
    </SortableSidebarMarker>
...
const SidebarThreadRow = memo(function SidebarThreadRow(props: {
  ...
  // and stays a passive cue while the descriptor is not loaded. Pinning
  // itself lives in the context menu.
  pinningSupported: boolean;
  isPinned: boolean;
  // Present on rows whose server supports drop outcomes: dnd-kit sortable bag
  // applied to the row root so the whole row drags (the pointer sensor's
                      ...
                      isWokeStatus ? (
                        ...
                        >
                          {topStatus.icon === "working" ? (
                            ...
                            // the mark is free to land the story it started.
                            <SidebarWorkingDuel animated={false} settled />
                          ) : null}
...
export default function Sidebar() {
  ...
  const {
    ...
    for (const thread of visible) {
      ...
      if (optimisticDrop?.key === threadKey) {
        ...
        const held = optimisticDrop.clearsSnooze
          ? projected
      ...
      // until Tuesday" temporarily suspends "keep on top". The pin survives
      // underneath — and so does its pinOrderKey, so on wake the thread
      // reappears at its exact slot in the pinned block.
      if (supportsSnooze && effectiveSnoozed(thread, { now: preciseNow })) {
        ...
        // Settlement outranks the pin: a finished thread belongs in the
        // settled shelf even while pinned, or the pinned block slowly fills
        // with work nobody is doing and stops being the "keep on top" list.
        // The pin is not lost — it still renders its marker, still holds its
        // pinOrderKey, and returns to the pinned block the moment the thread
        // is unsettled.
  ...
  const handleThreadFileDrop = useCallback(
    async (threadRef: ScopedThreadRef, files: File[]) => {
      ...
      if (destination === router.state.location.pathname) return;
      try {
        await navigateToThread(threadRef);
        if (destination !== router.state.location.pathname) clearPendingFileDrop(dropId);
      } catch {
  ...
  useEffect(() => {
    ...
    const canonicalSection = effectiveSnoozed(thread, { now: new Date().toISOString() })
      ? "snoozed"
      : threadNeedsYou(thread) || isBuildSystemThreadAwaitingUser(buildSystemRuns, thread.id)
        ? "needs-you"
        : thread.settledOverride === "settled"
          ? "settled"
          : thread.pinnedAt != null
            ? "pinned"
            : "active";
    const bouncedToNeedsYou =
      canonicalSection === "needs-you" &&
      (optimisticDrop.section === "active" || optimisticDrop.section === "pinned");
    if (
    ...
    if (canonicalSection !== optimisticDrop.section && !bouncedToNeedsYou) {
      return;
    }
    ...
    const keyByThread = new Map(
      canonicalDestination.map((row) => [
        scopedThreadKey(scopeThreadRef(row.environmentId, row.id)),
        (optimisticDrop.section === "pinned" ? row.pinOrderKey : row.activeOrderKey) ?? null,
      ]),
    );
  ...
  }, [activeKeys, buildSystemRuns, optimisticDrop, pinnedKeys, threads]);
  ...
  const sidebarListItems = useMemo((): readonly SidebarListItem[] => {
    ...
    if (needsYouThreads.length > 0) {
      items.push({ kind: "marker", marker: "needs-you-header" });
      items.push(...rowsOf(needsYouThreads, "needs-you"));
    }
                      ...
                      {(item: (typeof projectScopeItems)[number]) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {project ? (
                              <>
                                {/* No Tooltip here, unlike the rest of this batch:
                                    these buttons live inside an open popup, and
                                    nothing in the app nests a popup inside another
                                    one. The dropped `title` only repeated the
                                    `aria-label` that is still on the button, and the
                                    project it belongs to is named immediately to its
                ...
                <SortableContext items={sortableIds} strategy={sidebarSortingStrategy}>
                  ...
                  >
                    {(() => {
                      ...
                      const recordDepth = (threads: readonly EnvironmentThreadShell[]) => {
                        for (const row of groupSideChatsUnderParents(
                          groupBuildSystemThreadsUnderOrchestrator(threads, buildSystemRuns),
                        )) {
                      ...
                      recordDepth(activeThreads);
                      recordDepth(visibleSnoozedThreads);
                      recordDepth(renderedSettledThreads);
                      const renderThreadRowInner = (
                        ...
                        return (
                          <SidebarThreadRow
                            ...
                            thread={thread}
                            variant={rowVariant}
                            depth={depthByKey.get(threadKey) ?? 0}
                      ...
                      for (const item of sidebarListItems) {
                        ...
                        switch (item.marker) {
                          case "needs-you-header":
                            items.push(
                              <SidebarSectionHeader
                                key="needs-you-header"
                                marker="needs-you-header"
                                label={`Needs you (${needsYouThreads.length})`}
                              />,
                            );
                            break;
                          case "pinned-header":
