...
import { DraftHeroStarters, type DraftHeroStarter } from "./chat/DraftHeroStarters";
...
export default function ChatView(props: ChatViewProps) {
  ...
  const autoUpdateEnvironments = useMemo(
    () =>
      automaticEnvironment
        ...
        : [],
    [automaticEnvironment, environmentById, logicalProjectEnvironments],
  );
  ...
  useEffect(() => {
    ...
    const retained = new Set(pendingQuestionDraftKeys);
    const keys = new Set([
  ...
  }, [activeThreadId, environmentId, pendingQuestionDraftKeys]);
  ...
  const pickDraftStarter = useCallback(
    (starter: DraftHeroStarter) => {
      const composer = composerRef.current;
      if (!composer) return;
      if (!composer.insertTextAtEnd(starter.prompt, { ensureLeadingBoundary: true })) return;
      scheduleComposerFocus();
    },
  ...
  // Layout effect, not an effect: the anchor is released before the browser
  // paints the frame that would have shown the blank gap.
  useLayoutEffect(() => {
  ...
  // the sent message can stay near the top. Ronin only owns streaming adjustments
  ...
  const backgroundLivenessBannerItem = useMemo<ComposerBannerStackItem | null>(() => {
    ...
    return {
      ...
      actions: (
        <>
          {showViewAgents ? (
            <Button size="xs" variant="outline" aria-label="View agents" onClick={addAgentsSurface}>
              View
            </Button>
          ) : null}
          <Button
  ...
  const runWorkspaceCommand = useCallback(
    (
      ...
      /** A held-down shortcut should close one panel, not the whole stack. */
      isRepeat = false,
    ): boolean => {
      ...
      if (command === "thread.settle") {
        if (!isServerThread || !activeThreadRef || !supportsSettlement) return true;
      ...
      if (command === "rightPanel.close") {
        ...
        if (!isRepeat) closeRightPanelSurface(activeRightPanelSurface);
        return true;
      }
  ...
  useEffect(() => {
    const handler = (event: globalThis.KeyboardEvent) => {
      ...
      if (runWorkspaceCommand(command, terminalFocusOwner, event.repeat)) {
        event.preventDefault();
        event.stopPropagation();
      }
    };
  ...
  const onSend = async (
    ...
    if (failure === null && turnAttachmentsResult._tag === "Success") {
      const dispatchMessageContext = buildOutgoingMessageContext(
      ...
      const dispatchMessageContextFields =
        dispatchMessageContext === undefined
          ? {}
          : supportsInlineMessageContext
            ...
            : {
                text: serializeLegacyContextMessage({
                  text: outgoingMessageText,
                  records: dispatchMessageContext.records,
                }),
              };
      const bootstrap =
  ...
  const onChangeActivePendingUserInputCustomAnswer = useCallback(
    (
      ...
      if (!question || question.allowCustomAnswer === false) return;
  ...
  // another transient busy state in between. What remains inside `onSend` are the checks
  ...
  }, [composerDraftTarget, sendEnvMode, setDraftThreadContext, workLocallyResendReady]);
  ...
  useEffect(() => {
    if (pendingSidebarFileDrops.length === 0 || typeof composerDraftTarget === "string") return;
    if (
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's Ronin server to browse pull requests."
      />
      ...
      >
        ...
        <WorkspaceTopbar
          ...
          reserveNativeControls={
            isElectron && reserveTitleBarControlInset && !inlineRightPanelOwnsTitleBar
          }
        >
          ...
          >
            ...
            <div className="relative flex min-h-0 flex-1 flex-col bg-background">
              ...
              <MessagesTimeline
                ...
                onReplayTurn={paintOnlyDisplayedTimeline ? null : handleReplayTurn}
                onRevertToTurnCount={
                  paintOnlyDisplayedTimeline ? noopHeldRevert : onRevertTimelineTurn
                }
                onAskOnTheSide={
                  !paintOnlyDisplayedTimeline && isServerThread ? handleAskOnTheSide : null
                }
                isRevertingCheckpoint={!paintOnlyDisplayedTimeline && isRevertingCheckpoint}
                onImageExpand={onExpandTimelineImage}
                onFileOpen={paintOnlyDisplayedTimeline ? noopHeldAttachment : openFileAttachment}
                openingVideoAttachmentId={
                  paintOnlyDisplayedTimeline ? null : openingVideoAttachmentId
                }
                markdownCwd={
                  paintOnlyDisplayedTimeline
