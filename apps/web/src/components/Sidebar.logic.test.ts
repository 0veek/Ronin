...
describe("shouldRecedeSidebarThread", () => {
  ...
  it("recedes background work while keeping active rows prominent", () => {
    const input = {
...
describe("resolveProjectStatusIndicator", () => {
  ...
  it("surfaces the highest-priority actionable state across project threads", () => {
    ...
    ).toMatchObject({ label: "Pending Approval", dotClass: "bg-status-attention" });
  });
  ...
  it("prefers plan-ready over completed when no stronger action is needed", () => {
    ...
    ).toMatchObject({ label: "Plan Ready", dotClass: "bg-status-decision" });
  });
});
...
describe("resolveSidebarDropTarget", () => {
  ...
  it("rejects Needs you as a drop destination and skips its rows in orders", () => {
    const withNeedsYou: SidebarListItem[] = [
      { kind: "marker", marker: "needs-you-header" },
      { kind: "thread", key: "n1", section: "needs-you" },
      { kind: "marker", marker: "pinned-header" },
      { kind: "thread", key: "p1", section: "pinned" },
      { kind: "marker", marker: "pinned-divider" },
      { kind: "thread", key: "a1", section: "active" },
    ];
    expect(resolveSidebarDropTarget(withNeedsYou, "a1", "n1")).toBeNull();
    expect(resolveSidebarDropTarget(withNeedsYou, "n1", "p1")).toEqual({
      section: "pinned",
      pinnedOrder: ["p1", "n1"],
      activeOrder: ["a1"],
    });
    expect(resolveSidebarDropTarget(withNeedsYou, "n1", "a1")).toEqual({
      section: "active",
      pinnedOrder: ["p1"],
      activeOrder: ["a1", "n1"],
    });
...
describe("resolveSidebarDropVerb", () => {
  it("names the state change a cross-section drop performs", () => {
    ...
    expect(resolveSidebarDropVerb("needs-you", "pinned")).toBe("pin");
    expect(resolveSidebarDropVerb("needs-you", "settled")).toBe("settle");
    expect(resolveSidebarDropVerb("needs-you", "active")).toBeNull();
    expect(resolveSidebarDropVerb("active", "needs-you")).toBeNull();
  });
  ...
  it("stays silent for same-section reorders, no target, and the snoozed shelf", () => {
    ...
    expect(resolveSidebarDropVerb("needs-you", "needs-you")).toBeNull();
  });
});
