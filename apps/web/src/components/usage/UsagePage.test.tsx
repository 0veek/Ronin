...
vi.mock("../ui/kbd", () => ({ Kbd: "kbd" }));
vi.mock("../ui/scroll-area", () => ({ ScrollArea: "div" }));
vi.mock("../ui/sidebar", () => ({ SidebarInset: "div" }));
// Rendered as components rather than raw tags: both take props React would
// reject on a host element (`ariaLabel`, `current`).
vi.mock("../WorkspaceBreadcrumb", () => ({
  WorkspaceBreadcrumb: ({ children }: { children?: unknown }) => children,
  WorkspaceBreadcrumbItem: ({ children }: { children?: unknown }) => children,
}));
vi.mock("../shell/WorkspaceTopbar", () => ({ WorkspaceTopbar: "header" }));
vi.mock("./ProviderMark", () => ({ ProviderMark: "span" }));
vi.mock("./UsageProviderChart", () => ({
  UsageChartLegend: "div",
  UsageProviderChart: "div",
}));
...
beforeEach(() => {
  ...
  testState.useUsage.mockReturnValue({
    merged: {
      ...
      // Non-zero: the page renders its empty state for `records === 0`.
      records: 2,
      models: modelTotals,
      hourly: [
        {
