...
export function createSidebarSortingStrategy(input: {
  ...
  function project({ rects, activeIndex, overIndex }: Layout) {
    ...
    for (const [index, item] of items.entries()) {
      ...
      if (item.section === "pinned" || item.section === "active" || item.section === "needs-you")
    ...
    if (groups["needs-you"].length > 0) {
      marker("needs-you-header");
      projected.push(...groups["needs-you"]);
    }
    ...
    const heights = projected.map((item) => {
      ...
      const fallback =
        item.kind === "thread" &&
        (item.section === "pinned" || item.section === "active" || item.section === "needs-you")
          ? cardHeight
          : slimHeight;
