...
function AboutVersionTitle({ updateAvailable = false }: { readonly updateAvailable?: boolean }) {
  return (
    <span className="inline-flex items-center gap-2">
      {/* The version code sits on the label's baseline; the badge stays centred. */}
...
export function useSettingsRestore(onRestored?: () => void) {
  ...
  const changedSettingLabels = useMemo(
    () => [
      ...
      ...((settings.skills?.disabled?.length ?? 0) > 0 ? ["Agent skills"] : []),
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogPopup className="max-w-lg">
        ...
        <AlertDialogFooter>
          <Button variant="ghost" className="sm:mr-auto" onClick={onConfirm}>
            Use token by token
          </Button>
    ...
    <section className="space-y-3">
      <Collapsible open={open} onOpenChange={setOpen}>
        <CollapsibleTrigger className="group flex min-h-8 w-full items-center gap-2 px-3 sm:px-4">
          ...
          <ChevronRightIcon className="size-4 text-muted-foreground transition-transform duration-(--duration-base) group-data-panel-open:rotate-90" />
        </CollapsibleTrigger>
        <CollapsiblePanel>
...
export function GeneralSettingsPanel() {
  ...
  const agentInAppNotificationsEnabled = useUiStateStore(
    (state) => state.agentInAppNotificationsEnabled,
  );
  const setAgentInAppNotificationsEnabled = useUiStateStore(
    (state) => state.setAgentInAppNotificationsEnabled,
  );
    ...
    <SettingsPageContainer className="max-w-3xl">
      ...
      <SettingsSection title="General">
        ...
        <SettingsRow
          ...
          description="Show a toast when another agent finishes, fails, or needs approval while Ronin has focus. Per device."
          control={
            <Switch
              checked={agentInAppNotificationsEnabled}
              onCheckedChange={setAgentInAppNotificationsEnabled}
              aria-label="In-app notifications"
            />
          }
        />
        <AgentSoundsRow enabled={agentSoundsEnabled} onEnabledChange={setAgentSoundsEnabled} />
        {supportsAutoSettlement && settings.sidebarAutoSettleAfterDays !== null ? (
          <SettingsRow
            serverScoped
        ...
        <SettingsRow
          ...
          description="Default model for generated text like thread titles on connected devices with this provider. Source control settings can override it with a dedicated source control writer model."
          resetAction={
            isTextGenerationModelDirty ? (
              <SettingResetButton
