/**
 * Integrations settings - preferences for surfaces Ronin embeds rather than
          ...
          {sized ? (
            <div className="flex min-w-0 items-center gap-1">
              ...
              <Tooltip>
                <TooltipTrigger
                  render={
                    // `ghost` where upstream says `ghost-muted`: that variant
                    // came with the control-geometry pass this fork did not
                    // take (Batch 4, `9885a845c`). Ronin's `ghost` already sets
                    // `--control-icon-color: var(--muted-foreground)`, so an
                    // icon-only button renders identically.
                    <Button
                      size="icon-sm"
                      variant="ghost"
                      disabled={disabled}
