...
/**
 * This fork writes its default palettes as hex literals, but every role goes
 * through the canonical OKLCH form on the way into storage. Round a theme
 * through that same step so a stored-versus-passed comparison is about which
 * themes were written, not how their colors happen to be spelled.
 */
function withCanonicalColors<T extends { colors: Record<string, string> }>(theme: T): T {
  return {
    ...theme,
    colors: Object.fromEntries(
      Object.entries(theme.colors).map(([role, color]) => [role, canonical(color)]),
    ),
  };
}
...
describe("theme files", () => {
  ...
  it("merges a small user file onto the matching contrast-safe base palette", () => {
    ...
    expect(theme).toMatchObject({
      ...
      colors: {
        ...
        // Not canonical(): the generator never sets `placeholder`, so it comes
        // through untouched from the default role base, which is hex.
        placeholder: "#968d9f",
      },
    });
  });
  ...
  it("serializes a theme back into the importable file shape", () => {
    ...
    expect(JSON.parse(serialized)).toMatchObject({
      ...
      appearance: "light",
      colors: { accent: PAPER_THEME.colors.accent },
    });
  });
  ...
  it("suppresses sidebar artwork during a live custom-theme preview", () => {
    ...
    applyThemeColorPreview(PAPER_THEME.colors, "light");
  ...
  it("includes the dual-mode maintainer themes", () => {
    ...
    for (const theme of maintainerThemes) {
      ...
      expect(theme.variants?.[oppositeAppearance]?.accent).toMatch(/^oklch\(/);
  ...
  it("atomically replaces an imported collection and removes stale variants", () => {
    ...
    ).toEqual(replacement.map(withCanonicalColors));
  ...
  it("replaces collection entries even when their stored collection label is malformed", () => {
    ...
    expect(replaceCustomThemeCollection(collection.id, [replacement])).toEqual([
      withCanonicalColors(replacement),
    ]);
    expect(getCustomThemes()).toEqual([withCanonicalColors(replacement)]);
