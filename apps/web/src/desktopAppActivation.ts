  ...
  if (projectId === null) {
    try {
      ...
      return failure(
        ...
        errorMessage(error, "Ronin could not add the project."),
      );
    }
  }
  ...
  try {
    ...
    if (opened === null) {
      return failure(
        request.requestId,
        "thread-open-failed",
        "Ronin could not open a new thread for the project.",
      );
    }
    return {
      version: 1,
    ...
    return failure(
      request.requestId,
      "thread-open-failed",
      errorMessage(error, "Ronin could not open a new thread for the project."),
    );
  }
}
