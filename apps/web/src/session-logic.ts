...
export interface WorkLogEntry {
  ...
  /** Stable provider tool-call identity used to group lifecycle updates into one row. */
  toolCallId?: string;
...
/**
 * A streaming turn re-derives the whole work log on every activity that
 * arrives, and every rebuilt row is a new object that MessagesTimeline has to
 * treat as changed. Activities are immutable once emitted, so their derived
 * row is too — cache it and the untouched prefix keeps its identities.
 *
 * Only `providerBoundary` depends on the derivation options, so the cache
 * remembers which label map produced the entry and re-derives when a provider
 * instance is renamed.
 */
const derivedWorkLogEntryByActivity = new WeakMap<
  OrchestrationThreadActivity,
  {
    readonly providerLabelByInstanceId: WorkLogDerivationOptions["providerLabelByInstanceId"];
...
function toDerivedWorkLogEntry(
  ...
  if (cached && cached.providerLabelByInstanceId === options.providerLabelByInstanceId) {
    return cached.entry;
  }
  const payload =
    activity.payload && typeof activity.payload === "object"
  ...
  derivedWorkLogEntryByActivity.set(activity, {
    providerLabelByInstanceId: options.providerLabelByInstanceId,
    entry,
  });
  return entry;
}
