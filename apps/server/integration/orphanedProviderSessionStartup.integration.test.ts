...
const makePersistedRuntimeLayer = (dbPath: string) => {
  ...
  const ledger = ProviderSessionLedger.layer.pipe(Layer.provide(persistence));
...
const startupDependencies = Layer.mergeAll(
  ...
  // Ronin-only: startup starts the automation scheduler, and its provider
  // session directory is backed by a continuation ledger.
