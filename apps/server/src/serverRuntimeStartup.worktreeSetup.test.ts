...
const decodeWorktreeSetupSnapshot = Schema.decodeUnknownEffect(WorktreeSetupSnapshot);
...
it.effect("marks setups still recorded as running failed after a restart", () =>
  Effect.gen(function* () {
    ...
    const payload = yield* decodeWorktreeSetupSnapshot(command.activity.payload);
