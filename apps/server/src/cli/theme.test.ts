...
describe("t3 theme", () => {
  it.effect("writes a default theme when no settings file exists yet", () =>
    Effect.gen(function* () {
      ...
      yield* runCli(["theme", "set", "graphite", "--base-dir", baseDir]);
      assert.equal(readSettings(baseDir).defaultTheme, "graphite");
    }),
  );
  ...
  it.effect("preserves settings it does not recognise", () =>
    Effect.gen(function* () {
      ...
      assert.equal(settings.defaultTheme, "graphite");
  ...
  it.effect("honors T3CODE_HOME like the rest of the CLI", () =>
    Effect.gen(function* () {
      const baseDir = makeBaseDir();
      yield* runCli(["theme", "set", "graphite"]).pipe(
        Effect.provide(
  ...
  it.effect("refuses to write when the settings file cannot be read", () =>
    Effect.gen(function* () {
      ...
      const failure = yield* runCli(["theme", "set", "graphite", "--base-dir", baseDir]).pipe(
        Effect.flip,
      );
  ...
  // An id this build ships no palette for is rejected rather than written,
  // so `t3 theme set` cannot leave clients pointed at nothing.
  it.effect("rejects a theme id this build does not ship", () =>
    Effect.gen(function* () {
      const baseDir = makeBaseDir();
  ...
  it.effect("treats a bare id as an id even when a file shares its name", () =>
    Effect.gen(function* () {
      const baseDir = makeBaseDir();
      const cwdFile = NodePath.join(baseDir, "graphite");
