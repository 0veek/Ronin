...
export class DesktopAppUnreachableError extends Schema.TaggedErrorClass<DesktopAppUnreachableError>()(
  ...
  override get message(): string {
    return "Could not reach the Ronin desktop app. Start or update the desktop app on this machine, then run `t3 app` again. A running Ronin server is not enough.";
  }
}
...
export class DesktopAppRequestFailedError extends Schema.TaggedErrorClass<DesktopAppRequestFailedError>()(
  ...
  override get message(): string {
    return `Ronin could not open ${this.workspaceRoot} (${this.code}).`;
  }
}
...
const runAppCommand = Effect.fn("cli.app")(function* (flags: {
  ...
  yield* Console.log(`Opened ${workspaceRoot} in Ronin.`);
});
...
export const appCommand = Command.make("app", {
  ...
  Command.withDescription("Open a project in the running Ronin desktop app."),
