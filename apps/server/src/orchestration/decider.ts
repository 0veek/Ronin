...
function isImportedAgentSessionMessageId(messageId: string): boolean {
...
function pullRequestHostFromUrl(url: string, fallback: string): string {
  try {
  ...
  switch (command.type) {
    ...
    case "project.meta.update": {
      ...
      if (command.scripts !== undefined) {
        // Persisted IDs predate shortcut validation. Let users edit or remove
        // them without allowing another invalid ID to enter the project.
    ...
    case "thread.meta.update": {
      ...
      if (command.linkedPullRequest != null) {
        ...
        const host = pullRequestHostFromUrl(
          linked.url,
