...
export function resolveProviderCompatibility(
  ...
  const message =
    status === "broken"
      ? "This provider version is known to be incompatible with this Ronin release."
      : status === "unsupported"
        ? "This provider version is outside the supported range for this Ronin release."
        : status === "graceful"
          ? "This provider version has limited compatibility with this Ronin release."
          : null;
