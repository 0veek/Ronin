...
// The directory writes the active binding *and* mirrors it into the
// continuation ledger, so both repositories have to be present wherever a test
// builds a real one.
function makeSessionRepositoriesLayer<E, R>(
  persistenceLayer: Layer.Layer<SqlClient.SqlClient, E, R>,
) {
  return Layer.mergeAll(ProviderSessionRuntime.layer, ProviderSessionLedger.layer).pipe(
...
function makeFakeCodexAdapter(provider: ProviderDriverKind = CODEX_DRIVER) {
  ...
  const adapter: ProviderAdapterShape<ProviderAdapterError> = {
    ...
    ...(provider === CODEX_DRIVER
      ? { compaction: { type: "native" as const, start: compactThread } }
      : provider === CURSOR_DRIVER
        ? { compaction: { type: "slash-command" as const, command: "/compress" } }
...
function makeProviderServiceLayer(
  ...
  const sessionRepositoriesLayer = makeSessionRepositoriesLayer(SqlitePersistenceMemory);
  const directoryLayer =
    input.directory === undefined
      ? ProviderSessionDirectoryLive.pipe(Layer.provide(sessionRepositoriesLayer))
...
it.effect("ProviderServiceLive catches stopAll failures during shutdown", () =>
  Effect.gen(function* () {
    ...
    const directoryLayer = ProviderSessionDirectoryLive.pipe(
      Layer.provide(sessionRepositoriesLayer),
    );
...
it.effect("ProviderServiceLive keeps persisted resumable sessions on startup", () =>
  Effect.gen(function* () {
    ...
    const sessionRepositoriesLayer = makeSessionRepositoriesLayer(persistenceLayer);
    const directoryLayer = ProviderSessionDirectoryLive.pipe(
      Layer.provide(sessionRepositoriesLayer),
    );
    ...
    }).pipe(Effect.provide(sessionRepositoriesLayer));
...
const boundedListing = makeProviderServiceLayer({
  directory: {
    ...
    getLedgerEntry: () => Effect.die("ProviderService.listSessions does not use getLedgerEntry"),
    recordLedgerDelivery: () =>
      Effect.die("ProviderService.listSessions does not use recordLedgerDelivery"),
    listLedgerEntries: () =>
      Effect.die("ProviderService.listSessions does not use listLedgerEntries"),
    clearLedger: () => Effect.die("ProviderService.listSessions does not use clearLedger"),
  },
});
...
describe("agent browser access", () => {
  const startSessionWith = (enableAgentBrowserAccess: boolean, threadId: ThreadId) =>
    Effect.gen(function* () {
      ...
      // Ronin's ProviderService also resolves the continuation ledger, so the
      // directory needs both session repositories, not just the runtime one.
      const directoryLayer = ProviderSessionDirectoryLive.pipe(
