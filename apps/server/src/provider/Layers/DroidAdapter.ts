      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const acp = yield* makeDroidAcpRuntime({
            ...
            clientInfo: { name: McpProviderSession.MCP_SERVER_NAME, version: "0.0.0" },
            ...(mcpSession
              ? {
                  mcpServers: [
                    {
          ...
          const requestedStartModelId = droidModelSelection?.model
            ? resolveDroidAcpBaseModelId(droidModelSelection.model)
            : undefined;
          const boundModelId = yield* applyDroidAcpModelSelection({
            runtime: acp,
          ...
          const session: ProviderSession = {
            ...
            cwd,
            ...(boundModelId ? { model: resolveDroidAcpBaseModelId(boundModelId) } : {}),
            threadId: input.threadId,
            resumeCursor: {
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const requestedTurnModelId = turnModelSelection?.model
                ? resolveDroidAcpBaseModelId(turnModelSelection.model)
                : undefined;
              const currentModelId = yield* applyDroidAcpModelSelection({
                runtime: ctx.acp,
              ...
              const displayModel = currentModelId
                ? resolveDroidAcpBaseModelId(currentModelId)
                : undefined;
