    ...
    const sendTurn: CursorAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          // sendTurn does not hold the thread lock, so Stop can interrupt the
          // notification fiber before prompt() returns. Drain only while that
          // consumer is still alive; otherwise EventStreamBarrier never acks.
          const notificationFiber = ctx.notificationFiber;
          if (!ctx.stopped && notificationFiber !== undefined) {
            yield* ctx.acp.drainEvents.pipe(
              Effect.raceFirst(Fiber.await(notificationFiber).pipe(Effect.asVoid)),
            );
          }
