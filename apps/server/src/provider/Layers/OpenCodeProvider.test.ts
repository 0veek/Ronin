...
const OpenCodeRuntimeTestDouble: OpenCodeRuntimeShape = {
  ...
  createOpenCodeSdkClient: ({ cliSpec: _cliSpec, ...input }) => {
    // `cliSpec` is a fork-only field (Kilo drives this same runtime); the
    // assertions here are about server auth, and KiloProvider.test.ts owns the
    // spec-routing coverage.
