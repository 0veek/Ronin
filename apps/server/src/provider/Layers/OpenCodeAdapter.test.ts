...
it.layer(OpenCodeAdapterTestLayer)("OpenCodeAdapterLive", (it) => {
  ...
  it.effect("processes late assistant metadata without visiting completed turns", () =>
    Effect.gen(function* () {
      ...
      const historyEvents: Array<unknown> = [];
      ...
      runtimeMock.state.subscribedEvents = [
        ...
        { type: "message.part.updated", properties: { sessionID, part: step } },
        {
          type: "message.updated",
          properties: {
            sessionID,
        ...
        },
        {
          ...
          properties: {
            sessionID,
            info: { id: "current-message", role: "assistant", parentID: "prompt-message" },
          },
        },
        { type: "message.part.updated", properties: { sessionID, part } },
        { type: "message.part.updated", properties: { sessionID, part: step } },
      ...
      NodeAssert.deepEqual(
        events
          ...
          .filter((delta) => delta === "Current response" || delta === "zero"),
        ["Current response", "zero"],
      );
      NodeAssert.equal(
        events.filter(
          (event) => event.type === "item.completed" && String(event.itemId) === "current-part",
        ).length,
        1,
      );
      yield* adapter.stopSession(threadId);
