...
function buildCodexTurnInstructions(input: {
  ...
  readonly browserToolsAvailable?: boolean | RoninToolAvailability;
}): Pick<CodexTurnStartParamsWithCollaborationMode, "collaborationMode" | "additionalContext"> {
  const nativeMode = toNativeInteractionMode(input.interactionMode);
  ...
  return {
    collaborationMode: {
      ...
      settings: {
        ...
        developer_instructions: buildCodexDeveloperInstructions(nativeMode),
      },
    },
