...
/**
 * Key-order-independent JSON, so two permission requests describing the same
 * operation hash the same. Only used to build session approval keys, which
...
export function selectGrokPermissionOptionId(
  ...
  // Grok 4.6 often omits allow_always. Ronin still offers "Always allow this session".
...
export function makeGrokAdapter(grokSettings: GrokSettings, options?: GrokAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    /** Surface Grok plan.md as Ronin's proposed-plan card (while writing + on exit). */
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const started = yield* Effect.gen(function* () {
            ...
            // Capture plan into Ronin's proposed-plan UI and abandon the native gate so
