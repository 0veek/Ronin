 ...
 * release. At runtime the service refreshes it from the same file on this
 * repository's `main` via raw.githubusercontent.com, so a new model can leave
 * the legacy section with a commit to `main` instead of a release. Preference order is remote,
...
const MODEL_MANIFEST_URL =
  "https://raw.githubusercontent.com/0veek/Ronin/main/apps/server/src/provider/model-manifest.json";
