...
const handleStaticAndDevRequest = Effect.fn("handleStaticAndDevRequest")(
  function* (immutableBuildAssets: ReadonlySet<string>) {
    ...
    // Canonicalise before opening: a symlink out of the static root must not
    // be served just because its link path resolves inside it.
    ...
    let opened = canonicalFilePath ? yield* openStaticFile(canonicalFilePath) : null;
    if (!opened) {
      ...
      opened = canonicalIndexPath ? yield* openStaticFile(canonicalIndexPath) : null;
      if (!opened) {
        return HttpServerResponse.text("Not Found", { status: 404 });
      }
    }
