// This file mostly exists because we want dev mode to say "Ronin (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Ronin (Dev)" : "Ronin (Alpha)";
...
export function resolveMacBundleInfoPlistStrings(executableName) {
  return {
    ...
    NSDocumentsFolderUsageDescription: "Ronin reads project files you open in the desktop app.",
  };
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Ronin name.
