...
export class DesktopBackendConfiguration extends Context.Service<
  ...
  {
    // Build the local primary backend's start config. Reads the primary's
    // port/host/exposure from DesktopServerExposure. Can fail with PlatformError
    // because bootstrap token generation uses crypto.randomBytes under the hood.
...
export const make = Effect.gen(function* () {
  ...
  return DesktopBackendConfiguration.of({
    ...
    resolvePrimaryLabel: Effect.sync(() =>
