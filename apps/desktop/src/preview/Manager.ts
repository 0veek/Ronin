...
type FrameCaptureConsumer = "recording";

// Recording keeps only the activity lease that suspends background throttling
// while the guest is captured natively; nothing reads its frames off the main
// process any more.
...
const makeNativeOperations = Effect.fn("PreviewManager.makeOperations")(function* (
  ...
  // Unthrottling the window alone is not enough: a backgrounded guest keeps its
  // own throttle and hands the capture stream frozen frames.
  ...
  /** A guest that replaced another mid-capture needs the same exemption the original had. */
  ...
  const stopFrameCapture = Effect.fn("PreviewManager.stopFrameCapture")(function* (
    ...
    yield* SynchronizedRef.modifyEffect(frameCaptureSessionsRef, (sessions) =>
      Effect.gen(function* () {
        ...
        return [undefined, remainingSessions] as const;
      }),
  ...
  const deliverEvent = (
    eventKind: "state-change" | "recording-input" | "pointer-event",
    tabId: string,
  ...
  const openPictureInPicture = Effect.fn("PreviewManager.openPictureInPicture")(function* (
    ...
    // Picture-in-picture removed; kept as a no-op for bridge compatibility.
  });
  ...
  const attachListeners = Effect.fn("PreviewManager.attachListeners")(function* (
    ...
    const recordingInput = (_event: unknown, input: unknown): void => {
        ...
        wc.setWindowOpenHandler((details) => {
          ...
          runFork(
            attempt({ operation: "openPreviewWindow.normalizeUrl", tabId }, () =>
              normalizePreviewUrl(details.url),
            ).pipe(
    ...
    yield* SynchronizedRef.modifyEffect(frameCaptureSessionsRef, (sessions) =>
      Effect.gen(function* () {
        ...
        window.once("closed", () => {
          ...
          mainWindowCleanupFiber = runFork(stopAllRecordings().pipe(Effect.ignore));
        });
        return [undefined, sessions] as const;
      }),
  ...
  const registerWebviewUnlocked = Effect.fn("PreviewManager.registerWebviewUnlocked")(function* (
    ...
    if (replacedWebContentsId !== null) {
      ...
      clearPendingRecording(tabId);
      yield* Effect.all(
        [detachControlSession(replacedWebContentsId), detachListeners(replacedWebContentsId)],
        { concurrency: 2, discard: true },
      );
    }
  ...
  const startFrameCapture = Effect.fn("PreviewManager.startFrameCapture")(function* (
    ...
    yield* SynchronizedRef.modifyEffect(frameCaptureSessionsRef, (sessions) => {
      return Effect.gen(function* () {
        ...
        // A guest that refuses to unthrottle leaves the window exempt for nothing,
        // so the window-level change rolls back with it.
  ...
  const startRecording = Effect.fn("PreviewManager.startRecording")(function* (
    ...
    return yield* withTabLifecycleLock(
      ...
      Effect.gen(function* () {
        ...
        // Chromium can hand back an empty first frame while a hidden guest
        // warms its compositor; a discarded capture forces that frame before
        // the media stream attaches to the source.
        yield* attemptPromise(
          {
  ...
  const emitPointerEvent = Effect.fn("PreviewManager.emitPointerEvent")(function* (
    ...
    if (recording?.consumers.has("recording") && webContentsId != null) {
      yield* attempt({ operation: "recording.pointer", tabId: event.tabId }, () => {
        const contents = webContents.fromId(webContentsId);
        if (contents && !contents.isDestroyed()) {
          contents.send(RECORDING_POINTER_CHANNEL, event);
        }
      });
    }
