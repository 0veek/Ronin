...
export class DesktopWindow extends Context.Service<
  DesktopWindow,
  {
    ...
    /** Push a capture lifecycle event to the renderer. Only `started` reveals the window. */
...
export const make = Effect.gen(function* () {
  ...
  const dispatchRendererEvent = Effect.fn("desktop.window.dispatchRendererEvent")(function* (
    ...
    const dispatch = Effect.sync(() => {
      ...
      Effect.andThen(
        reveal ? electronWindow.reveal(targetWindow).pipe(Effect.ignoreCause) : Effect.void,
      ),
    );
    if (targetWindow.webContents.isLoadingMainFrame()) {
      const webContents = targetWindow.webContents;
      const sendOnce = () => void runPromise(dispatch);
      if (channel === MENU_ACTION_CHANNEL) {
        const previous = pendingMenuSendByContents.get(webContents);
        if (previous) webContents.removeListener("did-finish-load", previous);
        const dedupedSendOnce = () => {
          pendingMenuSendByContents.delete(webContents);
          sendOnce();
        };
        pendingMenuSendByContents.set(webContents, dedupedSendOnce);
        webContents.once("did-finish-load", dedupedSendOnce);
      } else {
