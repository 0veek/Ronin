...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "Ronin");
  expect(findCaptureDestination([window(99, "Ronin"), target], 42, "Ronin")).toBe(target);
  expect(findCaptureDestination([window(99, "Ronin")], 42, "Ronin")).toBeUndefined();
