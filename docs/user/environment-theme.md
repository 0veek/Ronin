...
the standard Ronin look. A theme you saved always wins over a published one with the same id.
...
A machine publishes themes by writing files into the `themes` directory of the Ronin state
...
exported from Ronin (the **Download** button's output) works as-is, so any theme someone shared
...
`appearance` is `light` or `dark`, and `canvas` and `accent` are hex colors. Ronin generates the
rest of the palette from those two, the same way the guided theme editor does, so the result reads
as a Ronin theme wearing your desktop's colors rather than a transplant of another app's.
...
out keep their generated value. A role this version of Ronin does not know is ignored rather
...
Write each file atomically — write a temporary file beside it and rename — so Ronin never reads
