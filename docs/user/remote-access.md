...
Use this when you want to connect to a Ronin server from another device such as a phone, tablet, or separate desktop app.
...
`t3 pair` finds the running server (the shared `~/.ronin` install, or the current worktree's dev server when run inside one), issues a one-time pairing token, and prints the pairing URL as a QR code you can scan from your phone.
...
run a headless server from the CLI, or have the desktop app launch Ronin over SSH.
...
In the **Add Environment** form, a numeric IP address without a scheme uses HTTP. Include `https://` explicitly when the backend is served over HTTPS.
...
Use this when you want the desktop app to start or reuse Ronin on another machine over SSH.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.ronin/ssh-launch/<host-key>/`, starts or reuses a remote T3 server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. Ronin uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, Ronin first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Ronin uses:
...
and they are saved separately in each client.
...
When the Ronin web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: Ronin may
...
see [Keeping Ronin in Sync](./updating.md).
...
Hosted pairing does not proxy traffic through Ronin. The browser still connects directly to the backend URL in the pairing link.
