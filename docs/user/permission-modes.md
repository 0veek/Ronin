...
threads do the same: the mode goes on Grok's command line, so **Supervised** starts Grok in ask
mode even if your Grok config is set to always-approve, and **Full access** starts Grok with
always-approve. The labels above describe what you get; the exact per-provider translation is
