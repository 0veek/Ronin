...
The same configuration lives in `~/.ronin/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. Ronin writes the built-in defaults into that file on
