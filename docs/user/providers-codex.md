...
This guide is for people who want to use more than one Codex account in Ronin. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Ronin](./install.md).
...
In Ronin Settings, name it something obvious:
...
In Ronin Settings, add another Codex provider:
...
Ronin shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Ronin treats it
