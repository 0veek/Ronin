...
Shift keys together. Ronin attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while Ronin is in front captures Ronin itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. Ronin refuses shortcuts
...
terminal contents. If an app is slow to answer, Ronin attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **Ronin SnapShots** extension during setup. It is installed
...
shortcut does not fire, check Ronin under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. Ronin shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. Ronin shows the exact binding it will
...
**Other Wayland desktops.** Ronin uses the desktop's screenshot portal when it can capture the
