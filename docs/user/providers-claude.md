...
This guide is for people who want to use more than one Claude setup in Ronin. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Ronin](./install.md).
...
In Ronin Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Ronin uses Claude Code's normal config directory.

When you set this field, Ronin points Claude Code at that directory with the
...
When you return to an older Claude thread with a large context, Ronin offers to compact the
...
Ronin looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where Ronin looks.

Then add another Claude provider in Ronin:
...
Ronin only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Ronin keeps separate config directories isolated
...
Add or edit a Claude provider in Ronin Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Ronin stores the value as a server secret and does not
...
Ronin does not need a special Claude Code Router provider. Treat the router as a Claude
